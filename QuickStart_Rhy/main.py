...
def qs_help():
    ...
    print(color_rep('    qs -v2gif path [sz] [fps] :-> generate gif from video'))
    print(color_rep('    qs -v2mp4 <video>         :-> format video to mp4'))
    print(color_rep('    qs -rmaudio <video>       :-> remove audio in video (return mp4 only)'))
    print(color_rep('    qs -upload                :-> upload your pypi library'))
    print(color_rep('    qs -upgrade               :-> update qs'))
    print('Docs:\n  -China: ' + Fore.BLUE + 'https://rhythmlian.cn/2020/02/14/QuickStart-Rhy/' + Style.RESET_ALL)
    print('  -Other: ' + Fore.BLUE + 'https://rhythmicc.github.io/2020/02/14/QuickStart-Rhy/' + Style.RESET_ALL)
